...
% 
%                                                                        
% X= (input_file.Curve1(2) - input_file.Curve1(1))+2;
%X = 1/Ts;
...
for n=1:t
    ...
    for i=1:min(n, N)
        for j=i:min(n, N)
            Cy = Cy + Cw2_cos(i,j)*cos(pi*i*X(n-j+1)*.5);
        end
    end
    
    for i=1:min(n, N)
        for j=i:min(n, N)
            Cy = Cy + Cw2_sin(i,j)*sin(pi*i*X(n-j+1)*.5);
        end
    end
    ...
    for j=1:min(n, N)
        for m=1:min([n-j, length(SP)])
           Xhx2_cos(1,j) = Xhx2_cos(1,j) +Shw(m)*cos(pi*i*X(n-m+1)*.5);
        end
    end
    ...
    for j=1:min(n, N)
        for m=1:min([n-j, length(SP)])
            Xhx2_sin(1,j) = Xhx2_sin(1,j) +Shw(m)*sin(pi*i*X(n-m+1)*.5);
        end
    end
